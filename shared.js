...
.app-shell-v2{display:flex;flex-direction:column;min-height:100vh;min-height:100dvh;background:var(--bg)}
.app-topbar-v2{position:sticky;top:0;z-index:100;background:var(--surface);display:flex;align-items:center;justify-content:space-between;padding:0 14px;height:48px;box-shadow:var(--shadow-sm);flex-shrink:0}
...
[data-theme="dark"]{
  --primary:#2dd4bf;--primary-soft:rgba(45,212,191,.15);--primary-dim:rgba(45,212,191,.08);
  --bg:#0f1117;--surface:#1a1d27;--surface-2:#252836;--border:#2d3148;
  --edge:rgba(255,255,255,.04);--text:#e2e8f0;--text-2:#94a3b8;--text-3:#64748b;
  --shadow-kpi:0 1px 3px rgba(0,0,0,.4),0 4px 14px rgba(0,0,0,.3);
  --shadow-panel:0 2px 4px rgba(0,0,0,.3),0 10px 28px rgba(0,0,0,.4);
  --shadow-sm:0 1px 2px rgba(0,0,0,.3),0 3px 8px rgba(0,0,0,.2);
  --ok:#4ade80;--ok-bg:#052e16;--info:#93c5fd;--info-bg:#0c1a2e;
  --warn:#fb923c;--warn-bg:#431407;--err:#fca5a5;--err-bg:#2d1515;
  --neutral:#94a3b8;--neutral-bg:#1e293b;
}
[data-theme="dark"] body{background:var(--bg);color:var(--text)}
[data-theme="dark"] .search-always{background:var(--surface);border-color:var(--border)}
[data-theme="dark"] .search-always input{color:var(--text)}
[data-theme="dark"] .search-always input::placeholder{color:var(--text-3)}
[data-theme="dark"] .fs-head-v2{color:var(--text-2)}
[data-theme="dark"] .fs-head-v2:hover{background:var(--surface-2)}
[data-theme="dark"] .filtre-banner{background:var(--info-bg);border-color:rgba(147,197,253,.3);color:var(--info)}
[data-theme="dark"] .filtre-banner button{background:var(--info)}
[data-theme="dark"] .filter-panel-v2{background:linear-gradient(180deg,var(--surface),var(--surface-2));border-color:var(--border);box-shadow:0 1px 0 rgba(255,255,255,.04) inset,0 2px 4px rgba(0,0,0,.3),0 10px 28px rgba(0,0,0,.3)}
[data-theme="dark"] .filter-head-v2{background:var(--surface)}
[data-theme="dark"] .filter-panel-v2.open .filter-head-v2{border-bottom-color:var(--border)}
[data-theme="dark"] .filter-head-v2-label{color:var(--text)}
[data-theme="dark"] .filter-btn-reset{color:var(--text-3);border-color:var(--border)}
[data-theme="dark"] .filter-expand-ico{color:var(--text-3)}
[data-theme="dark"] .filter-section-v2{border-bottom-color:var(--border)}
...
function VueHistorique({entries,onEdit,onDelete,onRefresh,onEntryUpdated,onDuplicate,initConseiller,onResetConseiller,canDelete,onChangeConseiller}){
  ...
  async function exportXLSX(){
    ...
    if(!window.XLSX){
      await new Promise((resolve,reject)=>{
        const s=document.createElement('script');
        s.src='vendor/xlsx-0.18.5/xlsx.full.min.js';
  ...
  function exportICS(){
    const evts=filtered.filter(e=>e.date).sort((a,b)=>String(a.date).localeCompare(String(b.date)));
    if(!evts.length){showToast&&showToast('Aucun atelier à exporter',false);return;}
    const blob=new Blob([buildICS(evts)],{type:'text/calendar;charset=utf-8'});
    const url=URL.createObjectURL(blob);
    const oris=[...new Set(evts.map(e=>(e.orienteur||'').trim()).filter(Boolean))];
  ...
  function printPDF(){
    ...
    const oris=[...new Set(filtered.map(e=>(e.orienteur||'').trim()).filter(Boolean))];
    const title=oris.length===1?'Ateliers — '+oris[0]:'Ateliers numériques';
    ...
    const w=window.open('','_blank');if(!w){showToast('Autorisez les popups pour ce site',false);return;}w.document.write(html);w.document.close();
  }
  ...
  return CE('div',null,
    ...
    CE('div',{className:'result-info-v2'},
      ...
      CE('button',{className:'btn-xlsx-v2',onClick:exportICS,title:'Exporter le calendrier (.ics) — compatible Google Calendar, Outlook, Apple Calendar',style:{background:'#3b82f6',color:'#fff',border:'none'}},
        CE('svg',{width:13,height:13,viewBox:'0 0 24 24',fill:'none',stroke:'currentColor',strokeWidth:2,strokeLinecap:'round',strokeLinejoin:'round'},CE('rect',{x:3,y:4,width:18,height:18,rx:2}),CE('path',{d:'M16 2v4M8 2v4M3 10h18'})),
        'ICS'
      ),
      CE('button',{className:'btn-xlsx-v2',onClick:printPDF,title:'Imprimer / Exporter en PDF',style:{background:'#e53e3e',color:'#fff',border:'none'}},
        CE('svg',{width:13,height:13,viewBox:'0 0 24 24',fill:'none',stroke:'currentColor',strokeWidth:2,strokeLinecap:'round',strokeLinejoin:'round'},CE('path',{d:'M6 9V2h12v7M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2M6 14h12v8H6z'})),
        'PDF'
      ),
...
// Une fois la date de l'atelier passée, plus rien à décider — le conflit
// bascule dans une section "Historique" séparée (grisée), sous les conflits
// encore actifs/à venir, plutôt que de rester mélangé avec eux.
    ...
    return CE('div',{className:printable?'frise-grid-wrap':undefined,style:{minWidth:jours.length*colWidth+140}},
      ...
      CE('div',{style:{display:'flex',flexDirection:'column',gap:colWidth<32?3:6}},
        pretsVisibles.map(p=>{
          ...
          // Aligné sur ateliers-cd47_NextStep le 19/09/2026.
...
// ── MAIN : VueDashboard ────────────────────────────────────
// Seule définition. Une version antérieure portant le même nom vivait plus
// haut dans ce fichier : les deux étant des déclarations de fonction au même
// niveau, la seconde écrasait silencieusement la première au chargement —
// celle-ci était donc déjà la seule active, l'autre n'était que du poids
// mort et une piste de lecture trompeuse. Supprimée le 19/09/2026.
