...
(function(){
  ...
  // script.googleusercontent.com/…/echo met encore 10 à 26 s avant de rendre le
  // corps (ou de répondre 404). Avec 12 s le retry était tué avant d'avoir
  ...
  window.apiFetch = async function apiFetch(action, body={}, _attempt=1){
    ...
    try{
      ...
      if(!res.ok){
        ...
        // redirection répond 404/429/5xx : c'est transitoire, pas un problème
        // de déploiement — d'où les 404 observés sur mobile. On réessaie.
        if(RETRYABLE_HTTP.indexOf(res.status) > -1){
      ...
      clearTimeout(timeoutId);
      const retryable = err.name === 'AbortError' || !!err.httpStatus;
