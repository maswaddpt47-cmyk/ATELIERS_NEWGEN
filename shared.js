...
(function(){
  ...
  window.apiFetch = async function apiFetch(action, body={}, _attempt=1){
    ...
    // Écritures et actions à effet de bord : jamais doublées — deux saveEntry
    // en parallèle créeraient un doublon. Une seule reprise, sur échec franc.
    try{
      return await gasUnLancer(url, action, _attempt, null);
    }catch(err){
